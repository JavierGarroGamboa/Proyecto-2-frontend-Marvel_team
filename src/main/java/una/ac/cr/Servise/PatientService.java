...
public class PatientService {
    ...
    public List<Patient> searchPatientsByTerm(int searchTerm) {
        ...
        if (patientsList != null && patientsList.size() > 0) {
            for (Patient patient : patientsList) {
                if (searchTerm > 0 && patient.getIdPatient() == searchTerm) {
    ...
    public Patient deletePatientsByTerm(int patient) {
        Patient patientDelete;

        patientDelete = client.target(REST_URI).path("/{id}")
                .resolveTemplate("id", patient).request(MediaType.APPLICATION_JSON).delete(Patient.class);
    ...
    public Patient upDatePatient(Patient patient) {
        ...
        patientUpdate = client.target(REST_URI).request(MediaType.APPLICATION_JSON).put(Entity.entity(patient,
                MediaType.APPLICATION_JSON), Patient.class);
