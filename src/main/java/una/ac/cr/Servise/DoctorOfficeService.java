...
public class DoctorOfficeService {
    ...
    public List<DoctorOffice> searchMedicalOfficesByTerm(int searchTerm) {
        ...
        if (medicalOfficeList != null && medicalOfficeList.size() > 0) {
            for (DoctorOffice medicalOffice : medicalOfficeList) {
                if (searchTerm > 0 && medicalOffice.getIdOffice() == searchTerm) {
    ...
    public DoctorOffice deleteDoctorOfficesByTerm(int patient) {
        DoctorOffice doctorOfficeDelete;

        System.out.println("Entro al Medical service a eliminar");
        doctorOfficeDelete = client.target(REST_URI).path("/{id}")
                .resolveTemplate("id", patient).request(MediaType.APPLICATION_JSON).delete(DoctorOffice.class);

        return doctorOfficeDelete;
    }
    ...
    public DoctorOffice upDateDoctorOffice(DoctorOffice patient) {
        ...
        doctorOfficeUpdate = client.target(REST_URI).request(MediaType.APPLICATION_JSON).put(Entity.entity(patient,
                MediaType.APPLICATION_JSON), DoctorOffice.class);
